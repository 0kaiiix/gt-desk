...
function generateInitialPedestals(): Pedestal[] {
  ...
  A_ZONE_CODES.forEach((columnList, colIdx) => {
    ...
    columnList.forEach((codeName, slotIdx) => {
      ...
      // A區第1排第5格：GT1222 (Jin) 舊位置 A區#1-5，新位置 A區#1-5
      if (customerId === 'GT1222' || (colNumber === 1 && slotNumber === 5)) {
        oldZone = 'A區';
        oldCol = 1;
        oldRow = 5;
        oldCode = 'A區#1-5';
  ...
  C_ZONE_CODES.forEach((columnList, colIdx) => {
    ...
    columnList.forEach((codeName, slotIdx) => {
      ...
      // C區第2排第1格：GT6211 (Kimber) 舊位置 C區#2-1，新位置 B區#6-6
      if (customerId === 'GT6211' || (colNumber === 2 && slotNumber === 1)) {
        oldZone = 'C區';
        oldCol = 2;
        oldRow = 1;
        oldCode = 'C區#2-1';
